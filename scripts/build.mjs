...
const url = (process.env.PUBLIC_SUPABASE_URL || process.env.SUPABASE_URL || '').trim().replace(/\/$/, '');
...
if (url && !/^https:\/\/[a-z0-9-]+\.supabase\.co(?:\.[a-z]{2,})?$/.test(url)) throw new Error('PUBLIC_SUPABASE_URL non valido: usare l’URL Project URL di Supabase, senza / finale.');
