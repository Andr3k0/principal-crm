...
test('database: workspace unico senza utenti o profili', async () => {
 const db = new PGlite();
 try {
  await db.exec("create role anon; create role authenticated; create schema auth; create function auth.uid() returns uuid language sql stable as $$select '11111111-1111-4111-8111-111111111111'::uuid$$; create function auth.jwt() returns jsonb language sql stable as $$select '{\"email\":\"info@principalsites.it\"}'::jsonb$$;");
